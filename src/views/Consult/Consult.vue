<template>
  <div class="app" ref="consult">
      ...
      <!--<div class="xw-header-content" @touchstart="showInfo">-->
        ...
        <!--</div>-->
      <!--</div>-->
      <mt-header title="在线咨询" class="gmkt-header">
        <!--<div @click="clickBack" slot="left">-->
          <!--<mt-button icon="back">返回</mt-button>-->
        <!--</div>-->
        <div class="xw-ring-wrap" slot="right">
          <a :class="[bellStatus ? '' : 'xw-ring-icon-toggle', 'xw-ring-icon']" @touchstart="hint('checkStaus')"></a>
        </div>
    ...
    <div class="xw-content" @touchstart="toShowMaskInfo=false"  ref="xwBody">
      <div class="xw-chat-wrap" >
        <ul class="xw-chat-ul">
          <li v-for="messageList in getMsgsRecords">
            <div v-if="messageList.type==2">
              <div class="xw-chat-time">{{messageList.createDT}}</div>
    ...
    <div class="xw-footer-wrap" @touchstart="toShowMaskInfo=false">
      <div class="xw-footer-content">
        <div class="xw-vmodel-wrap">
          <textarea class="xw-content-textarea input-for-ios" placeholder="请输入您的问题" v-model="content" @focus="onFocusText"></textarea>
        </div>
...
<script>
  ...
  export default {
    mounted () {
      this.hint('checkStaus')
      this.setXwBody(this.$refs.xwBody)
    ...
    created(){
      this._loadEmojiData();
      this.loadMsgs()
      this.scrollToBottom(this.$refs.xwBody)
    },
    watch: {
    },
    computed: {
      ...mapGetters(['getMsgsRecords','userInfo'])
    },
    methods: {
      ...mapMutations(['loadMsgs', 'setMsgsRecords', 'setXwBody']),
      clickBack () {
        this.$router.go(-1)
      },
      showInfo(){
      ...
      sendMsg(){
        ...
        param.content = this.content.trim()
        param.merchandiseId = null
        this.postRequest('submitMessage', param).then(res => {
...
//        setTimeout(() => {
//          //滚动条长度
//          let currentDistance = xwBody.scrollHeight - xwBody_.clientHeight;
//          //当前滚动条距离顶部的距离
