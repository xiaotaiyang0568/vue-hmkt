<template>
  <div class="main">
    <mt-header title="编辑收货地址" class="gmkt-header"></mt-header>
    <ul class="nav">
      <div class="nav-bac">
        <div class="nav-item">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveAddress () {
        this.postRequest('gcwwwifc/createOrUpdateExpressAddress', {
          ...
          if (res.data.code === 0) {
            this.$toast('修改成功')
            this.getProfile()
            this.$router.push({name: this.RouterName.expAddress})
          } else {
      ...
      removeAddress () {
        this.postRequest('gcwwwifc/deleteExpressAddress', {
      ...
      getProfile () {
        this.postRequest('gcwwwifc/getUserProfile?ProfileType='+util.ProfileType.expAddress).then(res => {
          if (res.data) {
